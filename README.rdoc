...
=== Creating an SDP description
  ...
  sdp = SDP::Description.new do
    session_description.add_field :version => 0
    session_description.add_field :media_description => { :media => "video" }
...
=== Parsing an SDP description
  ...
  sdp = SDP.parse sdp_string
  
  sdp.class         # => SDP::Description
  sdp[:session_description][:version]   # => 0
  sdp[:media_description][0][:media]    # => "audio"
  sdp[:media_description][0][:port]     # => 49170
  sdp[:media_description][0][:protocol] # => "RTP/AVP"
  sdp[:media_description][0][:format]   # => 0
  sdp[:media_description][1][:media]    # => "video"
  sdp[:media_description][1][:port]     # => 51372
  sdp[:media_description][1][:protocol] # => "RTP/AVP"
  sdp[:media_description][1][:format]   # => 99
