...
# Class for parsing SDP description text, ideally when receiving as some sort of client
# that uses SDP for describing the session for that protocol.
# 
# This parser _could_ raise an error when not finding a required SDP field, but was
# instead designed to not do so.  For example, if the parser parses the text from an
# SDP description, and that text is missing the Timing field (t=), the RFC 4566 implies
# that this description isn't valid (because that field is required).  Instead of
# raising an exception, the parser lets the SDP::Description class deal with this.
